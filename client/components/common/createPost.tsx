	...
	return (
		<div
			...
			onClick={(e: React.MouseEvent<HTMLDivElement>) =>
				(e.target as HTMLDivElement).id === "create-post" &&
			...
			<div className=" grid aspect-square h-full place-items-center overflow-hidden">
				<div className="aspect-square w-full rounded bg-white">
					...
					<div
						className={`mt-[-33px] h-full place-items-center ${
							index == 1 ? "grid" : "hidden"
						}`}>
						<div className="grid place-items-center gap-5">
							...
							/>
							<button
								className="rounded-md bg-violet-500 p-5 py-2 text-white"
								onClick={() =>
									document
					...
					<div
						className={`h-[calc(100%-49px)] w-full place-items-center ${
							index >= 2 ? "flex" : "hidden"
						} relative`}>
						...
						<div
							className={`h-full ${
								index == 3 ? "w-full" : "w-0"
							} space-y-2 transition-width duration-500  ease-in-out`}>
							<div className="p-2">
								<User
